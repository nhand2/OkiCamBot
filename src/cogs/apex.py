...
class ApexCommandsCog(commands.Cog, name='Apex Commands'):
    ...
    async def try_get_map(self, ctx):
        ...
        if self.message is not None and now <= self.currentMapEndTime:
            self.discordEmbed.set_field_at(index=0, name="Time Remaining", value="{:0>8}".format(str(timedelta(seconds=self.currentMapEndTime - now))))
            try:
        ...
        try: 
            ...
            if self.currentMapEndTime is not None and now < self.currentMapEndTime:
                self.discordEmbed.set_field_at(index=0, name="Time Remaining", value=currentMap['remainingTimer'])
            else:
            ...
            try:
                ...
                self.message = await ctx.send(content=f'Map Rotation', embed=self.discordEmbed)
    ...
    def get_from_apex(self):
        ...
        response = requests.get(
            f'https://api.mozambiquehe.re/maprotation?auth={Settings.APEX_API_KEY}&version=1', headers=header)
